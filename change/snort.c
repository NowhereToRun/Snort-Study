...
int SnortMain(int argc, char *argv[])
{
	...
	LogMessage("snort_main_thread_pid %d\n",snort_main_thread_pid);
#ifndef WIN32
    snort_main_thread_id = pthread_self();
#endif
    ...
    {
		LogMessage("DAQ_UnprivilegedStart\n");
        SnortUnprivilegedInit();
        SetPktProcessor();
        DAQ_Start();
    }
...
{
	//LogMessage("---------------GetPacketSource--------------------\n");
    const char* intf = "other";
	//int temp = ScReadMode();
	//int temp2 = !ScVersionMode() && !ScRuleDumpMode();
	//printf("ScReadMode %d\n",temp);
	//printf("ScVersionMode %d\n",temp2);
    if ( ScReadMode() )
    {
		//直接运行snort时 即嗅探器模式，在这里
        intf = PQ_First();
        PQ_Show(intf);
    }
